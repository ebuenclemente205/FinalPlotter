...
/*----------subject table-------------*/
	...
	function add_subject($edpcode, $subject, $stime, $etime, $days, $room, $units)
	{
		$db = db_plotter();
		$sql = "insert into subject(edpcode, subject, stime, etime, days, room, units) values(?,?,?,?,?,?,?)";
		$st = $db->prepare($sql);
		$st->execute(array($edpcode, $subject, $stime, $etime, $days, $room, $units));
		$db = null;
	}
...
/*----------plotter table-------------*/
	...
	function search_student_plotter($keysearch)
	{
		$keysearch = '%' . $keysearch . '%';
		$db = db_plotter();
		$sql = "select * from plotter where studentid like ?";
		$st = $db->prepare($sql);
		$st->execute(array($keysearch));
		$rows = $st->fetchAll();
		$db = null;
	...
	function cancel_student_plotter_subject($status,$plotterid)
	{
		$db = db_plotter();
	...
	function view_student_existing ($studentid)
	{
		$db = db_plotter();
		$sql = "select * from plotter where plotterid = ?";
		$st = $db->prepare($sql);
		$st->execute(array($studentid));
		$row = $st->fetch();
		$db = null;
	...
	function find_student_existing ($studentid, $status_p)
	{
		$db = db_plotter();
		$sql = "select * from plotter where studentid = ? and status = ?";
		$st = $db->prepare($sql);
		$st->execute(array($studentid, $status_p));
		$row = $st->fetch();
		$db = null;
...
/*----------pltted_subject table-------------*/		
	...
	function cancel_student_plotted_subject($plotterid)
	{
		$db = db_plotter();
		$sql = "delete from plotted_subject where plotterid = ?";
		$st = $db->prepare($sql);
		$st->execute(array($plotterid));
		$db = null;
	}
