<?php	
	...
	{			
		...
		{
			$plotterid = $_GET['id'];
			$check = view_student_existing($plotterid);
			$studentid = $check['studentid'];
			$display = find_student_existing($studentid, $status_a);
			if(count($display) > 1)
			{
				$error = 1;
				$message = "Studen's plotter is already approved.";
			}
			else
			{
				$error = 0;
				update_student_plotted_subject($status_a, $plotterid);
				$message = 'Student plotter approved!';
			}
		}
		...
		{
			$plotterid = $_GET['pid'];	
			cancel_student_plotter_subject($status_c, $plotterid);
			
			$error = 1;
			$message = 'Student plotter cancelled';
		}
    ...
    <header>
        <div class="container">
			...
			<div class="plotter-admin-bg">
				...
				<?php if(count($plotter_approved) > 0): ?>
					<div class="table-responsive table subject-table">
						<table class="table table-striped table-condensed table-hover">
							<thead>
								<tr>
